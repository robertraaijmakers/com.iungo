...
module.exports = class DriverCarChargerMeter extends Homey.Driver {
  async onInit() {}
  ...
  async onPairListDevices(state?: any, data?: any) {
    ...
    let result = [];
    const foundCarChargers = Object.values(foundDevices).filter((device) => device.type === 'evcharger');

    for (const carCharger of foundCarChargers) {
      let deviceObj = {
        name: carCharger.name,
        data: { iungo_id: state.iungo.id, id: carCharger.id },
        icon: '/icons/default.svg',
      };

      if (typeof iconsMap[carCharger.modelId as keyof typeof iconsMap] === 'string') {
        deviceObj.icon = `/icons/${iconsMap[carCharger.modelId as keyof typeof iconsMap]}.svg`;
      }
