...
module.exports = class DeviceEnergyMeter extends Homey.Device {
  ...
  syncDevice( ) {
    ...
    this.setAvailable()
		.catch(this.error)
		.then(this.log(`Setting device ${deviceInstance.name} avalable`));
    ...
    let deviceState = this.getState();
		...
		for(let capabilityId in capabilities) {
          ...
          // << duplicate flow card triggers. For inherited flow cards, (e.g. meter_power.xxxx) the flowCard with <capability_id>_changed will be automatically be triggered.
					switch(capabilityId) {
						case 'meter_power.t1':
      ...
      var deviceInstance = iungo.getEnergyMeter( deviceData.id );
			...
			if(oldValue !== value) {
        // For modbus devices (only) add the meter_power capability if still missing, for all other capabilities, add if missing
	...
	if(settings.length === 0) {
		...
		if(changed) {
			this.setSettings( deviceInstance.settings )
			.catch(this.error)
			.then(this.log(`Updated settings to ${deviceInstance.settings}`));
		}
	}
  }
