...
module.exports = class DeviceSocket extends Homey.Device {
  async onInit() {
    ...
    this.registerCapabilityListener('onoff', (value, opt) => {
      this.onCapabilitiesOnOffSet(value, opt);
    });
  }
  ...
  onCapabilitiesOnOffSet(value: any, opts: any) {
    this.log('onCapabilitiesOnOffSet', value, opts);

    // Sync (new) device name with Iungo
    let deviceData = this.getData();
    return (this.homey.app as IungoApp).save(deviceData.iungo_id, deviceType, deviceData.id, 'onoff', value);
  }
};
